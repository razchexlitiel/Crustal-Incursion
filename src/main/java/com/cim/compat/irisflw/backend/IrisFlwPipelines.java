...
public class IrisFlwPipelines {
   public static final Pipeline IRIS_INSTANCING = Pipeline.builder().compilerMarker("iris_instancing").vertexMain(ResourceUtil.rl("internal/iris_instancing/main.vert")).fragmentMain(ResourceUtil.rl("internal/iris_instancing/main.frag")).assembler(BufferTextureInstanceComponent::new).onLink((program) -> {
