...
import com.cim.compat.irisflw.backend.IrisInternalVertex;
...
public class IrisExtVertexView extends AbstractVertexView {
   ...
   public long stride() {
      return (long)IrisInternalVertex.EXT_STRIDE;
   }
   ...
   private int repackTangent(int packedTangent) {
      float x = NormI8.unpackX(packedTangent) + 1.0F;
      float y = NormI8.unpackY(packedTangent) + 1.0F;
      float z = NormI8.unpackZ(packedTangent) + 1.0F;
      float w = NormI8.unpackW(packedTangent) + 1.0F;
      return NormI8.pack(x, y, z, w);
   }

   private int repackMidBlock(int midBlock) {
      float x = (float)(midBlock & 255) * 0.015625F + 2.0F;
      float y = (float)(midBlock >> 8 & 255) * 0.015625F + 2.0F;
      float z = (float)(midBlock >> 16 & 255) * 0.015625F + 2.0F;
      int emission = midBlock >> 24 & 255;
      return (int)(x * 64.0F) & 255 | ((int)(y * 64.0F) & 255) << 8 | ((int)(z * 64.0F) & 255) << 16 | emission << 24;
   }

   static {
      STRIDE = (long)IrisInternalVertex.EXT_STRIDE;
   }
}
