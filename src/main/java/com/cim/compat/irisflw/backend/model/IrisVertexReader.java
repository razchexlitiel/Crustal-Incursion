...
public class IrisVertexReader implements VertexList {
   ...
   public Vector2i mcEntity(int index) {
      short renderEntity = MemoryUtil.memGetShort(this.ptr + (long)index * STRIDE + 32L);
      short renderedBlockEntity = MemoryUtil.memGetShort(this.ptr + (long)index * STRIDE + 34L);
   ...
   static {
      STRIDE = (long)IrisVertexFormats.TERRAIN.getVertexSize();
   }
}
