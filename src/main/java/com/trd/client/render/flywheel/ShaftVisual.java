...
public class ShaftVisual extends AbstractBlockEntityVisual<ShaftBlockEntity> implements SimpleDynamicVisual {
    ...
    private void rebuildRotor() {
        if (this.rotorInstance != null) {
            this.rotorInstance.delete();
            this.rotorInstance = null;
        }
    ...
    public void update(float pt) {
        ...
        if (blockEntity.getAttachedRotor().getItem() != this.currentRotorItem) {
            this.currentRotorItem = blockEntity.getAttachedRotor().getItem();
            rebuildRotor();
            updateLight(pt);
        }
...
// --- КОНЕЦ МАТЕМАТИКИ ---
    ...
    private void rebuildBelt() {
        ...
        MainRegistry.LOGGER.info("[BELT-DEBUG] rebuildBelt: orbitLength={}, trackCount={}, spacing={}", orbitTotalLength, trackCount, trackSpacing);
    }
